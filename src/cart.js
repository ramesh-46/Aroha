...
function Cart() {
  ...
  const handleApplyCoupon = async () => {
    try {
      setCouponError("");
      const res = await axios.post("http://localhost:5000/coupons/apply", { code: couponCode });
      if (res.data.success) {
        const coupon = res.data.coupon;
        if (coupon.category && coupon.category.trim() !== "") {
          const hasCategorizedItem = cart.items.some(item => 
            selectedItems.includes(item.productId._id) && 
            item.productId.category === coupon.category
          );
          if (!hasCategorizedItem) {
            setCouponError(`This coupon is only valid for items in the "${coupon.category}" category.`);
            return;
          }
        }
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(180px,1fr))", gap: "15px" }}>
          {cart.items.map(item => (
            ...
            >
              ...
              <img src={item.productId.images?.[0]?.startsWith("http") ? item.productId.images[0] : `http://localhost:5000/uploads/${item.productId.images?.[0]}`} alt={item.productId.name} style={imgStyle} />
      ...
      {selectedItems.length > 0 && (
        <div style={invoiceBox}>
          ...
          <div style={{ margin: "15px 0", display: "flex", gap: "10px", alignItems: "center" }}>
            ...
            />
            {appliedCoupon ? (
               <button style={cancelBtn} onClick={() => { setAppliedCoupon(null); setCouponCode(""); }}>Remove</button>
            ) : (
               <button onClick={handleApplyCoupon} style={{...orderBtn, marginTop: 0}}>Apply</button>
            )}
          </div>
          {couponError && <p style={{ color: "red", fontSize: "0.9em", textAlign: "left" }}>{couponError}</p>}
          {appliedCoupon && discountAmount > 0 && <p style={{ color: "green", textAlign: "left", fontWeight: "bold" }}>Discount Applied: -₹{discountAmount.toFixed(2)}</p>}
          
          <h3 style={{ marginTop: "10px" }}>Total Payable: ₹{finalAmountToPay.toFixed(2)}</h3>
...
const imgStyle = { width: "100%", height: "120px", objectFit: "contain", borderRadius: "5px" };
