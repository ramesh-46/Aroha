...
import {
  ...
  FaTshirt, FaChild, FaGem, FaShoppingBag, FaShoppingBasket, FaMale, FaFemale
} from "react-icons/fa";
...
function Dashboard() {
  ...
  // Fetch unique categories, subcategories, and brands
  useEffect(() => {
    axios.get("http://localhost:5000/products")
      .then(res => {
        ...
        setBrandsList(Array.from(bset));
      })
      .catch(err => console.log("Failed to fetch product metadata", err));
  }, []);
  ...
  const getCategoryIcon = (category) => {
    if (!category) return <FaTag size={24} />;
    const cat = category.toLowerCase();
    if (cat.includes("women") || cat.includes("girl")) return <FaFemale size={28} />;
    if (cat.includes("men") || cat.includes("boy")) return <FaMale size={28} />;
    if (cat.includes("child") || cat.includes("kid")) return <FaChild size={28} />;
    if (cat.includes("jewel") || cat.includes("ring")) return <FaGem size={28} />;
    if (cat.includes("accessor")) return <FaShoppingBag size={28} />;
    if (cat.includes("shirt") || cat.includes("cloth")) return <FaTshirt size={28} />;
    return <FaShoppingBasket size={28} />;
  };
  ...
  return (
    <div style={styles.container}>
      ...
      <main style={styles.main}>
        ...
        {/* Category Icons Row (Flipkart style) moved below search bar */}
        <div style={styles.categoryBarWrapper}>
          <div style={styles.categoryBar}>
            {[...new Set([...Object.keys(categoriesMap), "Children", "Jewelry"])]
              .filter(cat => !cat.toLowerCase().includes("test"))
              .map(cat => (
              <div 
                key={cat} 
                style={{
                  ...styles.categoryIconCircle, 
                  borderBottom: filters.category === cat ? "3px solid #2c3e50" : "3px solid transparent"
                }}
                onClick={() => {
                  toggleFilter("category", filters.category === cat ? "" : cat);
              ...
              >
                ...
                <span style={{...styles.categoryLabel, color: filters.category === cat ? "#2c3e50" : "#333", fontWeight: filters.category === cat ? "bold" : "500"}}>{cat}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {showFilters && (
          <div style={styles.filtersContainer}>
            {/* Category selection is handled by the visual icon bar now */}
                ...
                return (
                  <div key={p._id} style={styles.productCard}>
                    ...
                    <div style={styles.productImageContainer}>
                      {images.length > 0 ? (
                        <img
                          src={images[0].startsWith("http") ? images[0] : `http://localhost:5000/uploads/${images[0]}`}
                          alt={p.name}
                    ...
                    <div style={styles.productInfo}>
                      ...
                      {color.length > 0 && (
                        <div style={styles.colorOptions}>
                          {color.map((c, i) => (
                            <div key={i} style={styles.colorPill}>
                              <span
                                style={{
                                  ...styles.colorDot,
                              ...
                              />
                              <span style={styles.colorPillText}>{c}</span>
                            </div>
                          ))}
                        </div>
                      )}
